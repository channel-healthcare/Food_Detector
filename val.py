...
def run(
    ...
    if accuracy:
        # Print results
        LOGGER.info(s)
        acc, acc_cls = confusion_matrix.accuracy()
        pf = '%20s' + '%11i' * 2 + '%11.3g' * 5  # print format
        LOGGER.info(pf % ('all', seen, nt.sum(), mp, mr, map50, map, acc))

        # Print results per class
        if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
            for i, c in enumerate(ap_class):
                LOGGER.info(pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i], acc_cls[i]))
    else:
        # Print results
...
if __name__ == "__main__":
    startT = get_now_timestamp()
    print(f'Start timestamp = {startT}')
    opt = parse_opt()
    main(opt)
    endT = get_now_timestamp()
    print(f'End timestamp = {endT}')
    print(f'Elapsed time = {datetime.timedelta(seconds=endT-startT)}')
